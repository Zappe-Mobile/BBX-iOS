...
- (void)viewDidLoad
{
    ...
    else if ([leisureType isEqualToString:@"Vouchers"]) {
        
        self.navigationItem.titleView = [self.navigationController setTitleView:NSLocalizedString(@"Vouchers", nil)];
        [self setData:[DataManager loadAllLifestyleVouchersFromCoreData] WithType:@"Vouchers"];
    }
...
- (void)setData:(NSArray *)dataArray WithType:(NSString *)type
{
    ...
    else if ([type isEqualToString:@"Vouchers"]) {
        
        for (Vouchers * Object in dataArray) {
