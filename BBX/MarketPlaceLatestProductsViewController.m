//
//  MarketPlaceLatestProductsViewController.m
//  BBX
//
//  Created by Roman Khan on 13/05/14.
...
@interface MarketPlaceLatestProductsViewController ()
{
...
@implementation MarketPlaceLatestProductsViewController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.titleView = [self.navigationController setTitleView:NSLocalizedString(@"Latest Listing", nil)];
    ...
    [[RequestManager sharedManager]getMarketplaceLatestProductListingWithCountryId:[[NSUserDefaults standardUserDefaults]objectForKey:@"SELECTEDCOUNTRY"] WithPageNumber:@"1" WithPageLimit:@"100" WithCompletionBlock:^(BOOL result, id resultObject) {
        
        if (result) {
            
            [DataManager storeAllLatestProducts:[resultObject objectForKey:@"SellItem"] DataBlock:^(BOOL success, NSError *error) {
                
                if (success) {
                    
                    [SVProgressHUD dismiss];
                    [self setupData:[DataManager loadAllLatestProductsFromCoreData]];
                }
                else {
                    [SVProgressHUD dismiss];
                }
...
{
    for (LatestProducts * Object in array) {
        
        [arrayProductsId addObject:Object.sellId];
        [arrayProductsTitle addObject:Object.sellTitle];
